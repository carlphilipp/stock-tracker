...
public class Job {
	...
	public final void run() throws SchedulerException {
		...
		final JobDetail jobCurrency = JobBuilder.newJob(CurrencyJob.class).withIdentity("jobCurrency", Constants.CURRENCY).build();
		final Trigger triggerCurrency = TriggerBuilder
			.newTrigger()
			.withIdentity("triggerCurrency", Constants.CURRENCY)
			.withSchedule(
