...
<body>
	<div id="refresh" class="reveal-modal">
		...
		<form id="sendRefresh" name="sendRefresh" autocomplete="on">
			<input type="button" value="${language['LIST_HIDDEN_CONFIRM']}" onclick="javascript:checkForm('sendRefresh', 'refresh','refreshButton',refreshList)">
	...
	<div id="addFollow" class="reveal-modal">
		...
		<form id="sendFollow" name="sendFollow" autocomplete="on">
				<table>
					...
					<tr>
					<td colspan="2"><input type="button" value="Add" onclick="javascript:checkForm('sendFollow','addFollow','hiddenButtonFollow',addFollow)">
	...
	<div id="modifyFollow" class="reveal-modal">
		...
		<form id="sendModifyFollow" name="sendModifyFollow" autocomplete="on">
				<table>
					...
					<tr>
						<td>${language['LIST_HIDDEN_LOWERLIMIT']}:</td>
						<td><input id="lower" name="lower" type="text" pattern="\d+(\.\d+)?" placeholder="Pattern: \d+(\.\d+)?"></td>
					</tr>
					<tr>
						<td>${language['LIST_HIDDEN_HIGHERLIMIT']}:</td>
						<td><input id="higher" name="higher" type="text" pattern="\d+(\.\d+)?" placeholder="Pattern: \d+(\.\d+)?"></td>
					</tr>
					<tr>
					<td colspan="2"><input type="button" value="${language['LIST_HIDDEN_CONFIRM']}" onclick="javascript:checkForm('sendModifyFollow','modifyFollow','hiddenButtonModifyFollow',modifyFollow)">
							<input id="hiddenButtonModifyFollow" type="submit" style="display: none;" value=""> </td>
					</tr>
				</table>
			</form>
			<form name="sendFollowDelete" id="sendFollowDelete">
			or <a href="#" id="deleteFollow" onClick="if(confirm('Are you sure you want to delete this company from your list?')) execFunWithTimeout(checkForm('sendFollowDelete','deleteFollow','processDeleteFollow',deleteFollow))">delete</a>
			...
			<div class="share_container">
				...
				<table id="tableList" border="1" class="shadow">
					...
					<c:forEach var="follow" items="${follows}">
						<tr>
							<td><fmt:formatDate value="${follow.company.lastUpdate }" pattern="yyyy-MM-dd" /><br> <fmt:formatDate value="${follow.company.lastUpdate }"
									pattern="HH:mm" /></td>
							<td><span class="bold">${follow.company.name}</span><br>(${follow.company.yahooId}) [<a href="javascript:poufpouf('${follow.company.yahooId}')">${language['LIST_INFO']}</a>] 
							[<a href="#" data-reveal-id="modifyFollow" onclick="javascript:updateFollow('${follow.id }', '${follow.company.yahooId}','${follow.lowerLimit }','${follow.higherLimit }')">${language['LIST_MODIFY']}</a>]
								<span id="${follow.company.yahooId}" class="companyInfo" style="display: none;"> <br> ${follow.company.sector}<br> ${follow.company.industry}<br>
							...
							<td class="tdRight"><c:set var="val" value="${follow.company.quote * 25 /follow.company.yearHigh }" />
						...
						<td class="tdRight"><c:if test="${!empty follow.lowerLimit}">
								...
								<c:choose>
									<c:when test="${follow.gapLowerLimit < 5 }">
										<span class="cQuoteUp"><fmt:formatNumber type="number" maxFractionDigits="1" value="${follow.gapLowerLimit}" />%</span>
									</c:when>
									<c:otherwise>
										<fmt:formatNumber type="number" maxFractionDigits="1" value="${follow.gapLowerLimit }" />%
									</c:otherwise>
								</c:choose>
							</c:if></td>
						<td class="tdRight"><c:if test="${!empty follow.higherLimit}">
								...
								<c:choose>
									<c:when test="${follow.gapHigherLimit < 5 }">
										<span class="cQuoteDown"><fmt:formatNumber type="number" maxFractionDigits="1" value="${follow.gapHigherLimit }" />%</span>
									</c:when>
									<c:otherwise>
										<fmt:formatNumber type="number" maxFractionDigits="1" value="${follow.gapHigherLimit }" />%
									</c:otherwise>
								</c:choose>
							</c:if></td>
						</tr>
