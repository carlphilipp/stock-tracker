				...
				<table id="tableAccounts" border="1" class="shadow tablesorter">
					...
					<c:forEach var="account" items="${portfolio.accounts }">
						<tr>
							...
							<td class="tdCenter">[<a href="#" data-reveal-id="modifyAccount" onclick="javascript:updateAccount('${account.id}', '${account.name}', '${account.currency }', '${account.liquidity}', '${account.del }');">${language['ACCOUNTS_MODIFY']}</a>]
							</td>
						</tr>
					</c:forEach>
					</tbody>
